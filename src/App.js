...
export default ()=>{
  ...
  const handleOpenMenu = ()=>{setShowNewMenu(status)}
  const [clients, setClients] = useState([])
 const handleShowApointments =()=>{setShowApointments(true)}
  ...
  useEffect(()=>{
        fetch('http://127.0.0.1:3200/clients')
        .then(res => {
            return res.json()
        })
        .then(data=>{
...
<div className='AppWindow'>
  ...
  <div className='SideBar'>
    <Menu show={shownewmenu} setShow={setShowNewMenu}/>
    <NewClient show={shownewclient} setShow={setShowNewClient}/>
    <Apointments show={showapointments} setShow={setShowApointments}/>
    ...
    <header> 
        ...
        <div className='HeaderButtons'>
            <div onClick={handleShowApointments} className='HeaderBNTHome'> 
    ...
    <div className='ClientList'>
        {clientsFiltered.map((item,key)=>(
          ...
          key={key}
          data = {item}
          active= {activeclient.id === clients[key].id}
          onClick={()=>setActiveClient(clients[key])}
          />
        ))}
    </div>
  </div>
    ...
    <div className='ContentArea'>
       ...
       <h2>Faça seus agendamentos e tenha controle total do seu dia!</h2>
       </div>
       }
    </div>
</div>
